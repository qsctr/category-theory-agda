...
module _ {ℓo ℓa ℓs ℓ≈ₛ} where

  record StructuredSetCategory : Set (suc (ℓo ⊔ ℓa ⊔ ℓs ⊔ ℓ≈ₛ)) where
    field
      Obj : Set ℓo
      _—→_ : Obj → Obj → Set ℓa
      ...
      _∘-fun_ : {A B C : Obj} → (G : B —→ C) → (F : A —→ B) →
        fun (G ∘ F) ≡ fun G Fun.∘ fun F
      id : (A : Obj) → (A —→ A)
      id-fun : {A : Obj} → fun (id A) ≡ Fun.id

  structuredSetCategory : StructuredSetCategory → Category ℓo ℓa (ℓs ⊔ ℓ≈ₛ)
  structuredSetCategory SSC = record
    { Obj = Obj
    ; _—→_ = _—→_
    ...
    ; isCategory = record
      ...
      ; unitʳ = λ {A} {B} F x → Setoid.reflexive (obj-setoid B)
        ...
        in begin
          ...
          fun F x              ∎ } }
    where
    open StructuredSetCategory SSC
...
Pos {c} {ℓ₁} {ℓ₂} = structuredSetCategory record
 { Obj = Poset c ℓ₁ ℓ₂
 ; _—→_ = PosetHomomorphism
 ; obj-setoid = Poset.Eq.setoid
 ; fun = PosetHomomorphism.⟦_⟧
 ; ≈-homo = PosetHomomorphism.Eq.isRelHomomorphism
 ; _∘_ = flip RelMorComp.posetHomomorphism
 ; _∘-fun_ = λ G F → ≡.refl
 ; id = RelMorId.posetHomomorphism
 ; id-fun = ≡.refl }
...
module _ where
  ...
  module Cat where
    ...
    module _ {C D : Category 0ℓ 0ℓ 0ℓ} (F G : Functor C D) where
      ...
      module _ (Fₒ≗Gₒ : F.Fₒ ≗ G.Fₒ) {A B : C.Obj} where
        ...
        rewrite-dom-cod-cong : D._≈ₐ_ {F.Fₒ A} {F.Fₒ B} =[ rewrite-dom-cod ]⇒
          D._≈ₐ_ {G.Fₒ A} {G.Fₒ B}
    ...
    Cat .isCategory .assoc {_} {_} {_} {C₄} H G F = (λ A → ≡.refl) ,
      λ {A} {B} f → IsEquivalence.refl
        (H.Fₒ (G.Fₒ (F.Fₒ A)) C₄.—→-equiv H.Fₒ (G.Fₒ (F.Fₒ B)))
      where
      module C₄ = Category C₄
      module F = Functor F
      module G = Functor G
  ...
  Mon {c} {ℓ} = structuredSetCategory record
    { Obj = Monoid c ℓ
    ; _—→_ = λ M N →
  ...
  module _ {a b ℓ₁ ℓ₂} {M : Monoid a ℓ₁} {N : Monoid b ℓ₂} where
    ...
    IsFunctor→IsMonoidHomomorphism : {Fₒ : ⊤ → ⊤} →
      (Fₐ : M.Carrier → N.Carrier) →
      IsFunctor (Monoid→Category M .isCategory) (Monoid→Category N .isCategory)
        Fₒ Fₐ →
    ...
    IsMonoidHomomorphism↔IsFunctor : (f : M.Carrier → N.Carrier) →
      IsMonoidHomomorphism f ↔
        IsFunctor (Monoid→Category M .isCategory)
          (Monoid→Category N .isCategory) Fun.id f
